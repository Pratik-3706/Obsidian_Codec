...
def api_convert() -> Any:
    ...
    if custom_output:
        ...
        if operation == "extract_audio":
            out_ext = "." + data.get("audio_codec", "mp3").replace("libmp3lame", "mp3").replace("libopus", "opus").replace("libvorbis", "ogg")
    ...
    try:
        if operation == "convert":
            ...
            cmd.append(output_path)
            friendly_encoder = f"Audio Extractor ({audio_codec.replace('libmp3lame', 'mp3').replace('libopus', 'opus')})"
            ...
            if img_mode == "single":
                ...
                cmd = ["ffmpeg", "-y", "-ss", str(timestamp), "-i", input_path, "-vframes", "1", "-q:v", "2", output_path]
            elif img_mode == "interval":
                ...
                cmd = [
                    "ffmpeg", "-y", "-ss", str(start), "-t", str(gif_duration),
                    "-i", input_path,
