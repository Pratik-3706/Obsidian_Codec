...
def display_file_probe(file_path: str) -> Optional[Dict[str, Any]]:
    ...
    for s in info["video_streams"]:
        streams_table.add_row(
            str(s["index"]), "VIDEO", s["codec_name"].upper(), f"{s['width']}x{s['height']} | {s['r_frame_rate']} fps"
        )
...
def run_ffmpeg_with_cli_progress(cmd: List[str], duration: float, output_path: str) -> None:
    ...
    t = threading.Thread(target=run_ffmpeg_subprocess, args=(job_id, cmd, duration, output_path, None), daemon=True)
    t.start()
    ...
    with Progress(
        ...
        while True:
            ...
            progress.update(task, completed=job["progress"], speed=job["speed"], size=job["size"])
...
def build_ffmpeg_convert_cmd(
    ...
    v_args, mapped_vcodec, hw_type = map_codec_and_build_args(
        vcodec=vcodec, preset=preset, crf=crf, resolution=resolution, hw_accel="auto"
    )
    ...
    if mapped_vcodec != vcodec:
        console.print(
            f"[bold green]GPU Acceleration Active:[/bold green] Auto-switched [cyan]{vcodec}[/cyan] to [cyan]{mapped_vcodec}[/cyan]!"
        )
...
def run_command_watch(args: argparse.Namespace) -> None:
    ...
    console.print(
        Panel(
            f"Starting directory watch on [cyan]{watch_dir}[/cyan]\nOutput: [green]{out_dir}[/green]\nDone: [yellow]{done_dir}[/yellow]",
            border_style="cyan",
        )
    )
            ...
            try:
                ...
                for root, dirs, files in os.walk(watch_dir):
                    ...
                    dirs[:] = [
                        ...
                        for d in dirs
                        if not (
                            (d_path := os.path.abspath(os.path.join(root, d))) == out_dir
                            or d_path == done_dir
                            or d_path.startswith(out_dir + os.sep)
                            or d_path.startswith(done_dir + os.sep)
                        )
                    ]
                ...
                for f_path in candidates:
                    ...
                    if check_file_stable(f_path):
                        ...
                        cmd = build_ffmpeg_convert_cmd(
                            f_path, out_file, vcodec, acodec, crf, preset, resolution, None, None, "soft", meta
                        )
                        ...
                        console.print(
                            f"[yellow]File {rel_path} is not stable yet (still writing). Skipping...[/yellow]"
                        )
...
def run_interactive_wizard() -> None:
    ...
    if op_choice == "1":  # Convert
        ...
        vcodec = Prompt.ask(
            "Select video codec", choices=video_codec_choices, default=select_default(video_codec_choices, "libx264")
        )
        ...
        acodec = Prompt.ask(
            "Select audio codec", choices=audio_codec_choices, default=select_default(audio_codec_choices, "aac")
        )
        ...
        while True:
            ...
            video_codec_choices, audio_codec_choices = compatible_codec_choices(container)
            vcodec = Prompt.ask(
                "Select video codec", choices=video_codec_choices, default=select_default(video_codec_choices, vcodec)
            )
            acodec = Prompt.ask(
                "Select audio codec", choices=audio_codec_choices, default=select_default(audio_codec_choices, acodec)
            )

        out_path = os.path.join(input_dir, f"{base_name}_obsidian.{container}")
        cmd = build_ffmpeg_convert_cmd(
            filepath, out_path, vcodec, acodec, crf, preset, resolution, None, None, "soft", info
        )
        run_ffmpeg_with_cli_progress(cmd, info["duration"], out_path)

    elif op_choice == "2":  # Extract Audio
        acodec = Prompt.ask(
            "Select audio codec", choices=["libmp3lame", "aac", "flac", "pcm_s16le", "libopus"], default="libmp3lame"
        )
        ...
        try:
            ...
            console.print(
                Panel(
                    f"[bold green]✔ Successfully Generated![/bold green]\nThumbnail sheet: [cyan]{out_path}[/cyan]",
                    border_style="green",
                )
            )
...
def main() -> None:
    ...
    if use_new_parser:
        ...
        convert_parser.add_argument(
            "--resolution", default="original", help="Output frame resolution (e.g. 1920x1080 or original)"
        )
        ...
        convert_parser.add_argument(
            "--sub-mode", choices=["soft", "hard"], default="soft", help="Soft embed or hard burn subtitles"
        )

        # 3. Extract subparser
        extract_parser = subparsers.add_parser("extract", help="Extract streams or components")
        extract_parser.add_argument(
            "type", choices=["audio", "video", "subs", "chapters"], help="Type of component to extract"
        )
        ...
        extract_parser.add_argument(
            "--sub-format", choices=["srt", "vtt"], default="srt", help="Subtitle format to extract"
        )
        ...
        watch_parser.add_argument(
            "-o", "--output", help="Directory path to save converted files (default: directory/converted)"
        )
        watch_parser.add_argument(
            "-d", "--done", help="Directory path to move completed original inputs (default: directory/done)"
        )
        ...
        if args.command == "probe":
            ...
            is_valid, err_msg = validate_transcoding_combination(
                out_container, args.vcodec, args.acodec, meta, args.audio_track
            )
            if not is_valid:
            ...
            if args.type == "audio":
                codec = args.acodec
                out_ext = "." + codec.replace("libmp3lame", "mp3").replace("pcm_s16le", "wav").replace(
                    "libopus", "opus"
                )
                if not output_path:
        ...
        parser.add_argument(
            "-interactive", "--interactive", action="store_true", help="Launch terminal wizard interactive UI"
        )
        ...
        parser.add_argument(
            "--sub-mode", choices=["soft", "hard"], default="soft", help="Mux mode for subtitles (soft or hard)"
        )
        ...
        if not args.input:
            console.print(
                "[bold red]Error:[/bold red] Input file (-i/--input) is required unless in interactive mode.",
                style="red",
            )
            sys.exit(1)
